...
const LevelUpModal = ({ newLevel, onClose }) => {
  ...
  <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-xl p-6 animate-in zoom-in duration-500" onClick={onClose}>
    ...
    <div className="text-center">
      ...
      <h1 className="text-5xl font-black text-white mb-2 text-transparent bg-clip-text bg-gradient-to-b from-yellow-300 to-orange-500 drop-shadow-sm">{t('lvl_up_title')}</h1>
      <p className="text-2xl text-white font-bold mb-8">{t('lvl_up_msg', [newLevel])}</p>
      <div className="bg-white/10 backdrop-blur-md p-6 rounded-3xl border border-white/20 max-w-xs mx-auto mb-8">
        <h3 className="text-white font-black mb-2 flex items-center justify-center gap-2"><Lock className="text-red-400" size={20}/> {t('lvl_up_new_items')}</h3>
        <p className="text-slate-300 text-sm">{t('lvl_up_check_store')}</p>
      </div>
