  ...
  return (
    <div className="animate-in slide-in-from-bottom-10 fade-in duration-300 pb-24 md:pb-0">
      ...
      <div className="bg-white/90 backdrop-blur-sm p-6 rounded-3xl border-b-4 border-slate-200 shadow-xl mb-6 flex flex-col items-center text-center relative overflow-hidden">
        ...
        <div className="w-24 h-24 bg-yellow-300 rounded-full border-4 border-white shadow-lg flex items-center justify-center text-5xl relative z-10 mt-8 mb-4">
          👨‍🌾
          <div className="absolute -bottom-2 bg-blue-600 text-white text-xs font-bold px-3 py-1 rounded-full border-2 border-white">
            {t('profile_lvl')} {level}
          </div>
        </div>
        ...
        <div className="w-full max-w-xs mt-4 relative z-10">
          <div className="flex justify-between text-xs font-bold text-slate-500 mb-1">
            <span>{t('profile_xp_current')}</span>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        <div className="bg-white/80 p-4 rounded-2xl border-b-4 border-slate-200">
          <div className="text-slate-400 text-xs font-bold uppercase mb-1 flex items-center gap-1">
            <Egg size={12}/> {t('stats_eggs')}
          </div>
          <div className="text-2xl font-black text-slate-700">{stats.total_eggs}</div>
        </div>
        <div className="bg-white/80 p-4 rounded-2xl border-b-4 border-slate-200">
          <div className="text-slate-400 text-xs font-bold uppercase mb-1 flex items-center gap-1">
            <Trash2 size={12}/> {t('stats_cleaned')}
          </div>
          <div className="text-2xl font-black text-slate-700">{stats.total_cleaned}</div>
        </div>
        <div className="bg-white/80 p-4 rounded-2xl border-b-4 border-slate-200">
          <div className="text-slate-400 text-xs font-bold uppercase mb-1 flex items-center gap-1">
            <DollarSign size={12}/> {t('stats_earned')}
          </div>
          <div className="text-2xl font-black text-slate-700">{stats.total_earned}</div>
        </div>
        <div className="bg-white/80 p-4 rounded-2xl border-b-4 border-slate-200">
          <div className="text-slate-400 text-xs font-bold uppercase mb-1 flex items-center gap-1">
            <Clock size={12}/> {t('stats_days')}
          </div>
          <div className="text-2xl font-black text-slate-700">{stats.days_played}</div>
        </div>
      </div>
      ...
      <div className="bg-white/90 backdrop-blur-sm p-5 rounded-3xl border-b-4 border-slate-200 shadow-xl pb-20">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-black text-slate-800 flex items-center gap-2">
            <Trophy className="text-yellow-500"/> {t('trophy_gallery')}
          </h3>
              ...
              <div key={achievement.id} className={`flex items-center gap-4 p-3 rounded-2xl border-2 transition-all ${isUnlocked ? 'bg-yellow-50 border-yellow-200' : 'bg-slate-50 border-slate-100 grayscale opacity-60'}`}>
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-slate-400 leading-tight">
                    {t(`achievement_${achievement.id}_desc`)}
                  </p>
                </div>
                {isUnlocked && <CheckCircle size={20} className="text-green-500"/>}
              </div>
            );
          })}
        </div>
