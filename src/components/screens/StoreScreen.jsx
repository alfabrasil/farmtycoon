  ...
  return (
    <div className="animate-in slide-in-from-bottom-10 fade-in pb-24 md:pb-0">
      ...
      <div className="flex gap-2 mb-4 bg-white/80 p-1 rounded-2xl overflow-x-auto no-scrollbar">
        ...
        <button onClick={()=>setTab('ITEMS')} className={`flex-1 py-2 px-3 rounded-xl font-black text-sm transition-all whitespace-nowrap ${tab==='ITEMS'?'bg-blue-400 text-white shadow-sm':'text-slate-400'}`}>ITENS</button>
        <button onClick={()=>setTab('TECH')} className={`flex-1 py-2 px-3 rounded-xl font-black text-sm transition-all whitespace-nowrap ${tab==='TECH'?'bg-purple-500 text-white shadow-sm':'text-slate-400'}`}>TECH</button>
        <button onClick={()=>setTab('STRUCTURE')} className={`flex-1 py-2 px-3 rounded-xl font-black text-sm transition-all whitespace-nowrap ${tab==='STRUCTURE'?'bg-slate-700 text-white shadow-sm':'text-slate-400'}`}>OBRAS</button>
        <button onClick={()=>setTab('DECOR')} className={`flex-1 py-2 px-3 rounded-xl font-black text-sm transition-all whitespace-nowrap ${tab==='DECOR'?'bg-pink-500 text-white shadow-sm':'text-slate-400'}`}>SKINS</button>
      </div>
        ...
        : tab === 'TECH' ? (
          ...
          <>
             <div className="bg-pink-100 p-4 rounded-3xl mb-4 text-center border-2 border-pink-200">
               <h3 className="font-black text-pink-700 flex items-center justify-center gap-2"><PaintBucket size={18}/> Construtor da Fazenda</h3>
               <p className="text-xs text-pink-600">Personalize a aparência da sua fazenda.</p>
             </div>
             {Object.values(SKINS_CONFIG).map(skin => {
               ...
               return (
                 <div key={skin.id} className={`bg-white/90 p-4 rounded-3xl border-b-4 flex items-center gap-4 ${isActive ? 'border-green-500 bg-green-50' : 'border-slate-200'}`}>
                    ...
                    <div className="flex-1">
                      <h3 className="font-black text-slate-800">{skin.name}</h3>
                      <p className="text-xs text-slate-500">Tema Visual Completo</p>
                    </div>
                    <button 
                    ...
                    >
                      {isActive ? 'EM USO' : skin.price === 0 ? 'USAR' : `${skin.price} 💰`}
                    </button>
                 </div>
               )
             })}
