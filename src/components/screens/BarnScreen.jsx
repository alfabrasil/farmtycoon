...
const BarnScreen = ({ onBack, inventory, onSellEggs, addFloatingText, marketPrices, upgrades, marketNews, goldenEggs, marketHistory }) => {
  ...
  return (
    <div className="animate-in slide-in-from-right-10 fade-in pb-24 md:pb-0">
      <div className="flex items-center gap-2 mb-4"><button onClick={onBack} className="bg-white p-2 rounded-full shadow-md"><X size={24}/></button><h1 className="text-2xl font-black text-slate-800 bg-white/50 px-3 py-1 rounded-xl">{t('barn_title')}</h1></div>
      
      {/* GRÁFICO DE MERCADO (COMMODITIES) */}
      <div className="bg-white/90 p-4 rounded-3xl border-b-4 border-slate-200 mb-4 shadow-sm">
        <h3 className="font-black text-slate-700 flex items-center gap-2 mb-2 text-xs uppercase"><Activity size={14}/> {t('barn_price_history')}</h3>
        ...
        <div className="flex justify-between text-[10px] text-slate-400 mt-1 font-bold">
           <span>{t('barn_days_ago')}</span>
           <span>{t('barn_today')}</span>
        </div>
      </div>

      {marketNews && (<div className={`mb-4 border px-4 py-3 rounded-xl flex items-center gap-3 ${marketNews.multiplier >= 1 ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}><div className="text-2xl">{marketNews.icon}</div><div className="flex-1"><h4 className={`font-black text-sm ${marketNews.color}`}>{t(`news_${marketNews.id}_title`)}</h4><p className="text-xs text-slate-600">{t(`news_${marketNews.id}_desc`)}</p></div></div>)}
      <div className="flex flex-col gap-2 mb-4">
        {upgrades?.WASHER && (<div className="bg-cyan-50 border border-cyan-200 text-cyan-800 px-4 py-2 rounded-xl flex items-center gap-2 text-sm font-bold"><CheckCircle size={16} /> {t('barn_washer_active')}</div>)}
        {upgrades?.PACKER && (<div className="bg-indigo-50 border border-indigo-200 text-indigo-800 px-4 py-2 rounded-xl flex items-center gap-2 text-sm font-bold"><Package size={16} /> {t('barn_packer_active')}</div>)}
        {goldenEggs > 0 && (<div className="bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-2 rounded-xl flex items-center gap-2 text-sm font-bold"><Sparkles size={16} /> {t('barn_prestige_bonus', [Math.round((prestigeMultiplier - 1)*100)])}</div>)}
      </div>
      <div className="bg-white/90 backdrop-blur-sm rounded-3xl p-6 border-b-8 border-orange-200 shadow-xl mb-6">
        <div className="flex justify-between items-center mb-4"><h2 className="text-xl font-black text-orange-800 flex items-center gap-2"><Egg /> {t('barn_stock_quote')}</h2><span className="text-[10px] font-bold bg-blue-100 text-blue-600 px-2 py-1 rounded-md flex items-center gap-1"><RefreshCcw size={10}/> {t('barn_update_sleep')}</span></div>
        <div className="space-y-3 mb-6">
          <div className="flex justify-between items-center p-3 bg-orange-50 rounded-xl border border-orange-100"><div className="flex items-center gap-3"><span className="text-2xl">{ITEMS_CONFIG.EGG_COMMON.icon}</span> <div><span className="font-bold text-slate-700 block text-sm">{t('item_EGG_COMMON_name')}</span><span className={`text-xs font-bold ${getPriceColor(marketPrices.common, BASE_PRICES.EGG_COMMON)}`}>{getArrow(marketPrices.common, BASE_PRICES.EGG_COMMON)} ${marketPrices.common}</span></div></div><div className="font-black text-lg">x{inventory.eggs_common}</div></div>
          <div className="flex justify-between items-center p-3 bg-orange-50 rounded-xl border border-orange-100"><div className="flex items-center gap-3"><span className="text-2xl">{ITEMS_CONFIG.EGG_RARE.icon}</span> <div><span className="font-bold text-slate-700 block text-sm">{t('item_EGG_RARE_name')}</span><span className={`text-xs font-bold ${getPriceColor(marketPrices.rare, BASE_PRICES.EGG_RARE)}`}>{getArrow(marketPrices.rare, BASE_PRICES.EGG_RARE)} ${marketPrices.rare}</span></div></div><div className="font-black text-lg">x{inventory.eggs_rare}</div></div>
          <div className="flex justify-between items-center p-3 bg-purple-50 rounded-xl border border-purple-100"><div className="flex items-center gap-3"><span className="text-2xl animate-pulse">{ITEMS_CONFIG.EGG_LEGENDARY.icon}</span> <div><span className="font-bold text-purple-700 block text-sm">{t('item_EGG_LEGENDARY_name')}</span><span className={`text-xs font-bold ${getPriceColor(marketPrices.legendary, BASE_PRICES.EGG_LEGENDARY)}`}>{getArrow(marketPrices.legendary, BASE_PRICES.EGG_LEGENDARY)} ${marketPrices.legendary}</span></div></div><div className="font-black text-lg text-purple-700">x{inventory.eggs_legendary}</div></div>
        </div>
        <div className="bg-slate-800 rounded-2xl p-4 text-white flex justify-between items-center mb-4"><span className="font-bold text-slate-400 uppercase text-xs">{t('barn_est_value')}</span><span className="font-black text-2xl text-yellow-400">💰 {totalValue}</span></div>
        <button onClick={() => { if(totalEggs > 0) { onSellEggs(totalValue); playSound('success'); } }} disabled={totalEggs === 0} className="w-full bg-green-500 hover:bg-green-600 disabled:bg-slate-300 text-white py-3 rounded-xl font-black shadow-md border-b-4 border-green-700 active:border-b-0 active:translate-y-1 transition-all disabled:border-slate-400">{t('barn_sell_all')}</button>
      </div>
      <div className="grid grid-cols-2 gap-4 mb-6"><div className="bg-white/90 p-4 rounded-3xl border-b-4 border-slate-200 flex flex-col items-center"><span className="text-4xl mb-2">{ITEMS_CONFIG.FEED.icon}</span><span className="font-black text-slate-700">{t('barn_feed')}</span><span className="text-sm font-bold text-slate-500">{t('barn_silo_cap', [inventory.feed, upgrades?.SILO ? 100 : 20])}</span></div><div className="bg-white/90 p-4 rounded-3xl border-b-4 border-slate-200 flex flex-col items-center"><span className="text-4xl mb-2">{ITEMS_CONFIG.VACCINE.icon}</span><span className="font-black text-slate-700">{t('barn_vaccines')}</span><span className="text-sm font-bold text-slate-500">{t('barn_doses', [inventory.vaccine])}</span></div></div>
    </div>
  );
};
