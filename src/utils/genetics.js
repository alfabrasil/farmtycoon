...
export const calculateBreedingResult = (parent1Type, parent2Type) => {
  ...
  if (roll < 0.10) {
    newType = 'MUTANTE'; // 10% chance de Mutante Puro (Global)
  } 
  // 2. Herança Cyber (Se um dos pais for Cyber)
  else if (roll < 0.25 && (parent1Type === 'CYBER' || parent2Type === 'CYBER')) {
    newType = 'CYBER'; // 15% chance extra se tiver pai Cyber (0.10 a 0.25)
  } 
  // 3. Hibridismo (Se pais forem de espécies diferentes)
  else if (parent1Type !== parent2Type && roll < 0.60) {
    newType = 'HIBRIDA'; // ~35-45% chance de Híbrida (0.25 a 0.60)
  } 
